...
function getClientNamesUrl() {
    var baseUrl = $("meta[name=baseUrl]").attr("content")
    return baseUrl + "/api/party/clients";
}

function getCustomerNamesUrl() {
    var baseUrl = $("meta[name=baseUrl]").attr("content")
    return baseUrl + "/api/party/customers";
}
...
function displayOrderList(data) {
    ...
    for (var i in data) {
        ...
        var invoiceButtonHtml = '<button style="margin-right:2px;" id="invoicebutton' + e.id + '" class="btn btn-primary btn-sm" onclick="getInvoice(' + e.id + ')"><i class="fa fa-print"></i>&nbspInvoice</button>';
